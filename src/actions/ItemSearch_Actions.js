    ...
    if (trimmedQuery === '') {
      ...
      if (trimmedQuery.length > 2) {
        ...
        seller.categories.map(category => {
          ...
          const ar_match = category.name.ar.toUpperCase().includes(trimmedQuery.toUpperCase());
          const en_match = category.name.en.toUpperCase().includes(trimmedQuery.toUpperCase());
        ...
        seller.categories.forEach(category => {
          ...
          category.sub.map(subcategory => {
            ...
            const ar_match = subcategory.name.ar.toUpperCase().includes(trimmedQuery.toUpperCase());
            const en_match = subcategory.name.en.toUpperCase().includes(trimmedQuery.toUpperCase());
            if (ar_match || en_match) {
              subcategoryResults.push({ ...subcategory, parent_name: category.name })
            }
          });
        });
      }
      ...
      algoliaIndex.search({
        ...
        console.log('ALL SEARCH', categoryResults, subcategoryResults, itemResults);
        dispatch({
          type: ITEM_SEARCH_DATA_SET,
          payload: {
            query,
