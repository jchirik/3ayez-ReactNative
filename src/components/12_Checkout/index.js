...
import { BackButton, BlockButton, LoadingOverlay, AyezText } from "../_common"
import { Row } from "./Row"
...
import { strings, formatTimestamp, formatDay, formatCurrency } from "../../i18n.js"
...
class Checkout extends Component {
    ...
    let creditToggle = (
      <TouchableOpacity
        ...
        <AyezText
          ...
          {strings("PaymentMethod.creditCard")}
        </AyezText>
        <View style={{ flex: 1 }} />
      ...
      balanceAppliedBanner = (
        <View
          ...
          <AyezText
            ...
            {strings("Checkout.appliedBalance", { applied: balance_applied.toFixed(2) })}
          </AyezText>
        </View>
      )
    }
        ...
        <Row disabled title={strings("Checkout.tipField")}>
          <View>
            <AyezText
              ...
              {strings("Checkout.smartTipSuggestions")}
            </AyezText>
            <FlatList
              data={tipData}
              horizontal={true}
  ...
  renderCouponReceiptRow() {
    ...
    return (
      <ReceiptRow
        title={strings("Receipt.coupon", { code: coupon.code })}
        cost={coupon.amount}
        color={"#D33B2D"}
      />
    )
  ...
  renderBalanceSubtractionRow() {
    ...
    return (
      <ReceiptRow
        title={strings("Receipt.balanceApplied")}
        cost={balance_applied}
        color={"#D33B2D"}
      />
    )
  }
  ...
  renderNotes() {
    ...
    if (!notes) {
      notesButtonText = strings("Common.add")
    }
  ...
  renderDeliveryTime() {
    const { seller, timeslot } = this.props

    if (timeslot && timeslot.type === "INSTANT") {
      return (
        <Row disabled title={strings("Checkout.deliveryTimeField")}>
          <View style={{ alignItems: "flex-end" }}>
            <AyezText regular size={16}>
              {strings("Common.yalla")}
            </AyezText>
            <AyezText regular size={13} color={"#8E8E93"}>
              {strings("StoreSelect.deliveryTime", { delivery_time: seller.delivery_time })}
            </AyezText>
          </View>
        </Row>
      )
    ...
    return (
      <View
        ...
        <ScrollView style={{ flex: 1 }}>
          ...
          <Row onPress={() => navigateBack()} title={strings("Checkout.addressField")}>
            <AyezText
              regular
              style={{
                fontSize: 16
              }}>
              {strings("Address.singleLine", {
        ...
        <View style={styles.submitButtonContainer}>
          <BlockButton text={strings("Checkout.sendOrder")} onPress={this.submitOrder.bind(this)} />
        </View>
